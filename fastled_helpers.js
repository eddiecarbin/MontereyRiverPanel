...
//import { adafruit_fancyled as fancy } from './adafruit_fancyled';
const fancy = require("./adafruit_fancyled").default;

var GFACTOR, __repo__, __version__;
__version__ = "0.0.0-auto.0";
__repo__ = "https://github.com/Adafruit/Adafruit_CircuitPython_FancyLED.git";
GFACTOR = 2.5;
function applyGamma_video(n, g_r = GFACTOR, g_g = null, g_b = null, inplace = false) {
    ...
    */
    if (((g_g !== null) && (g_b !== null))) {
        g_r = [g_r, g_g, g_b];
    }
    return fancy.gamma_adjust(n, g_r, { "inplace": inplace });
}
function napplyGamma_video(n, g_r = GFACTOR, g_g = null, g_b = null) {
...
function loadDynamicGradientPalette(src, size) {
    ...
    */
    var grad;
    grad = [];
    for (var i = 0, _pj_a = src.length; (i < _pj_a); i += 4) {
        grad.append([(src[i] / 255.0), new fancy.CRGB(src[(i + 1)], src[(i + 2)], src[(i + 3)])]);
    }
    return fancy.expand_gradient(grad, size);
}
function ColorFromPalette(pal, pos, brightness = 255, blend = false) {
    ...
    */
    var color;
    if (blend) {
        pos = ((pos / 16.0) / pal.length);
    } else {
        pos = (Math.floor((pos / 16.0)) / pal.length);
    }
    color = fancy.palette_lookup(pal, pos);
    if ((brightness < 1.0)) {
        brightness /= 255.0;
        if ((color instanceof fancy.CHSV)) {
            color = new fancy.CRGB(color);
        } else {
...
function hsv2rgb_spectrum(hue, sat, val) {
    ...
    RETURNS: CRGB color.
    */
    return new fancy.CRGB(new fancy.CHSV((hue / 255), (sat / 255), (val / 255)));
}
...
//# sourceMappingURL=fastled_helpers.js.map
